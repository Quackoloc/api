...
@Injectable()
export class MarkColocationTaskAsUndoneUseCase {
  constructor(
  ...
  async execute(colocationId: number, taskId: number, connectedUser: ConnectedUser): Promise<void> {
    ...
    colocationTask.completed = false;
    await this.colocationTaskRepository.save(colocationTask);

    logger.info(
      `Colocation task with id : ${taskId} in colocation with id : ${colocationId} marked as undone by user with id : ${connectedUser.id}`
    );
  }
}
